...
export default async function ProjectBoard({ params }: { params: { projectId: string } }) {
  ...
  async function createShare() {
    ...
    await supabase
      .from('public_shares')
      .insert({ project_id: params.projectId, token: crypto.randomUUID().replaceAll('-', '') });
  }
  ...
  return (
    <AppShell>
      <div className="mb-3 flex flex-wrap items-center gap-2">
        ...
        <Link href={`/app/projects/${params.projectId}/settings`} className="rounded border px-2 py-1">
          Settings
        </Link>
        <Link href={`/app/projects/${params.projectId}/zones`} className="rounded border px-2 py-1">
          Zones
        </Link>
        <Link href={`/app/projects/${params.projectId}/trains`} className="rounded border px-2 py-1">
          Trains
        </Link>
        <Link href={`/app/projects/${params.projectId}/analytics`} className="rounded border px-2 py-1">
          Analytics
        </Link>
        <Link href={`/app/projects/${params.projectId}/export`} className="rounded border px-2 py-1">
          Export
        </Link>
